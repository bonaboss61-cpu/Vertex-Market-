...
regex = r'<div className="grid grid-cols-2 gap-4 mt-2 p-3 bg-white/5 rounded-lg">[\s\S]*?</svg>\s*</div>\n\s*\)\)\n\s*\}'
replacement = """<div className="grid grid-cols-3 gap-4 mt-2 p-3 bg-white/5 rounded-lg">
...
code = re.sub(r'<div className="grid grid-cols-2 gap-4 mt-2 p-3 bg-white/5 rounded-lg">[\s\S]*?</div>\n\s*</div>\n\s*\)\)\n\s*\}', replacement, code)
